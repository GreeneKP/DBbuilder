...
#The below commented code is a concept to make textbox for picking a GEO sat a selectbox instead; caging user options, but decreasing likelihood of failure due to user input.
...
#
#satcat = satcat[satcat['PERIOD'] > 1240]
#satcat = satcat[satcat['PERIOD'] < 1640]
#
#collist = ['NORAD_CAT_ID','OWNER','LAUNCH_DATE','INCLINATION','APOGEE','PERIGEE']
#
#satcat.index = satcat['OBJECT_NAME']
#
#for col in satcat.columns:
#    if col not in collist:
#        satcat = satcat.drop(columns=col)
#
#
...
number_search = sat_selection_form.text_input('Select NORAD SATCAT Number of a specific Satellite you want in GEO: ')
...
#st.header("Available NORAD SATCAT (NORAD CAT ID) Reference")
#
#st.write(satcat)
...
#tenlist = [37810, 42814, 39476, 37749, 45920, is broken27718, 41589, 38991, 34111, 32487]
...
#died at 42815 due to remote disconnection
...
if submission:
    ...
    pickedscc = int(number_search)

    satlist.append(pickedscc)

    st.subheader(f'a Database of the following SCCs will be built:') 
    st.write(satlist)
    st.write(f'Database Building in progress... Please wait... Estimated wait from start time to completion: {len(satlist)*3.3} minutes')

    for i in range(len(satlist)):

            zr = get(f"https://www.space-track.org/basicspacedata/query/class/gp_history/NORAD_CAT_ID/{satlist[i]}/orderby/TLE_LINE1%20ASC/EPOCH/2000-01-01--{today}/format/tle",headers=hdrs2)
            oneline = zr.text
  ...
  #                      file.close
   ...
   #                     file.close
        ...
        #    st.write('Database pull for SCC not functioning. Moving on to next.')
    st.header('Database Successfully built!')